...
const ProductModal = ({
  ...
  const validateForm = () => {
    ...
    // For editing, if there's no original image and no new image
    if (product && !imageFile && !originalImage && !formData.images[0]?.trim())
      newErrors.images = "Image is required";
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add all fields for new products
      if (!product) {
        fd.append("name", formData.name.trim());
        fd.append("description", formData.description.trim());
        fd.append("price", String(Number(formData.price)));
        ...
        const currentSizes = JSON.stringify(formData.sizes.sort());
        const originalSizes = JSON.stringify(
          (product.sizes || ["S", "M", "L"]).sort()
        );
        if (currentSizes !== originalSizes) {
          fd.append("sizes", JSON.stringify(formData.sizes));
        }

        const currentDetails = JSON.stringify([...formData.details].sort());
        const originalDetails = JSON.stringify(
          [...(product.details || [])].sort()
        );
        ...
        const currentCommitment = JSON.stringify(
          [...formData.commitment].sort()
        );
        const originalCommitment = JSON.stringify(
          [...(product.commitment || [])].sort()
        );
        if (currentCommitment !== originalCommitment) {
          fd.append("commitment", JSON.stringify(formData.commitment));
        }
      }
      ...
      if (imageFile) {
        fd.append("image", imageFile);
      } else if (formData.images[0] && formData.images[0] !== originalImage) {
        // If the image URL has changed but no file uploaded
        fd.append("images", formData.images[0]);
      }
      ...
      if (!contentType || !contentType.includes("application/json")) {
        ...
        // If it's HTML, check for common error patterns
