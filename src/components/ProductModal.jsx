  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!product) {
        ...
        // For updates (PATCH), include only changed fields

        // Check name
        if (hasChanged(formData.name.trim(), product.name)) {
          fd.append("name", formData.name.trim());
        }

        // Check description
        if (hasChanged(formData.description.trim(), product.description)) {
          fd.append("description", formData.description.trim());
        }
        ...
        if (hasChanged(formData.categoryId, currentCategoryId)) {
          fd.append("categoryId", formData.categoryId);
        }

        // Check sizes
        if (hasChanged(formData.sizes, product.sizes || [])) {
          fd.append("sizes", JSON.stringify(formData.sizes));
        }

        // Check details
        if (hasChanged(formData.details, product.details || [])) {
          fd.append("details", JSON.stringify(formData.details));
        }

        // Check commitment
        if (hasChanged(formData.commitment, product.commitment || [])) {
          fd.append("commitment", JSON.stringify(formData.commitment));
        }
      ...
      if (product && product._id) {
        ...
        // If no fields were added (nothing changed), show message and return
